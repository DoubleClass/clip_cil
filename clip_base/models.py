...
def sample(mean, cov, size, mahalanobis_range=None, nearest=None, shrink=False):
    vec = torch.randn(size, mean.shape[-1], device=mean.device)
    if mahalanobis_range is not None:
        vec = vec / vec.norm(dim=-1, keepdim=True)

        vec = vec * (torch.rand(size, 1, device=mean.device) * (mahalanobis_range[1]-mahalanobis_range[0]) + mahalanobis_range[0])
    # 使用均值协方差进行变换
    if shrink:
        cov = shrink_cov(cov)
...
class ClassIncremental(nn.Module):
    def __init__(self, cfg, device, classnames, jit=False):
        ...
        if cfg.model_name2 == 'RN50' or cfg.model_name2 == 'RN101':
            # self.model, self.transforms = clip.load(cfg.model_name2, device=device, jit=jit)
            self.model, _, self.transforms = open_clip.create_model_and_transforms('ConvNext-Base', pretrained='/home/ubuntu/code/the_old_good/vit_b_16-laion400m_e32-55e67d44.pt')
            self.model = self.model.to(self.device)
        elif cfg.model_name2 == 'ViT-B/16' or cfg.model_name2=='SUP21K':
        ...
        if cfg.model_name2 == 'SUP21K':
            self.image_encoder = ViT('B_16_imagenet1k', pretrained=True).to(torch.float16).to(self.device)

        # import pdb; pdb.set_trace()
        self.task_inc = cfg.task_inc
        ...
        if self.has_attention:
            ...
            # just linear probe
            if cfg.model_name2 == 'ViT-B/16':
                self.classifer = ClipMapping(512, cfg.initial_increment).to(self.device)
            else:
                self.classifer = ClipMapping(1024, 10).to(self.device)
        elif cfg.task_cls:
            # just linear probe
            if cfg.model_name2 == 'ViT-B/16':
        ...
        #     self.classifer = ClipMapping(512, 10).to(self.device)
            ...
            self.classifer = ClipMapping(512, 10).to(self.device)
        elif cfg.model_name2 == "ViT-B/16" and not cfg.fc2:
            self.clip_mapping = ClipMapping(feat_in=512, num_vector_dim=512)
            self.clip_mapping = self.clip_mapping.to(self.device)
        elif cfg.model_name2 == "RN101" and not cfg.fc2:
            self.clip_mapping = ClipMapping(feat_in=512, num_vector_dim=512)
            self.clip_mapping = self.clip_mapping.to(self.device)
        elif cfg.model_name2 == "SUP21K":
            self.clip_mapping = ClipMapping(feat_in=768, num_vector_dim=512)
            self.clip_mapping = self.clip_mapping.to(self.device)
        elif cfg.fc2:
            self.clip_mapping = ClipMapping2fc(feat_in=512, num_vector_dim=512)
            self.clip_mapping = self.clip_mapping.to(self.device)
        else:
            self.clip_mapping = ClipMapping(feat_in=1024, num_vector_dim=1024)
            self.clip_mapping = self.clip_mapping.to(self.device)
        # self.act = nn.GELU()
        ...
        # import pdb; pdb.set_trace()
        self.model_type = cfg.model_name
        self.model_type2 = cfg.model_name2
        # import pdb; pdb.set_trace()
    
    def forward(self, image, ori_ima_f=False, is_train=True, task_id=0, targets=[], memory_data=None):
        if self.linear_probe:
            # linear probe, just using a fc layer to classify
            if self.model_type2 != 'ViT-B/16':
        ...
        if self.reuse != 0:
            ...
            #     import pdb; pdb.set_trace()
            with torch.no_grad():
                if self.model_type2 == 'RN50' or self.model_type2 == 'RN101':
                    image_features = self.image_encoder(image.type(self.dtype))
                elif self.model_type2 == 'SUP21K':
                    # import pdb; pdb.set_trace()
                    logits, image_features = self.image_encoder(image.type(self.dtype))
            ...
            fea_for_encoded = fea_for_encoded / fea_for_encoded.norm(dim=-1, keepdim=True)
            text_features = self.text_features / self.text_features.norm(dim=-1, keepdim=True)
            logit_scale = self.logit_scale.exp()
            logits = logit_scale * fea_for_encoded @ text_features.t()
            if not ori_ima_f:
                image_features = fea_for_encoded
            return logits, image_features
        if self.ood and self.task_inc and is_train:
            image_features, in_fea = self.image_encoder(image.type(self.dtype))
            fea_for_encoded = image_features.type(torch.FloatTensor).to(self.device)
            fea_for_encoded_new = self.classifer(fea_for_encoded)
            fea_for_encoded_new = fea_for_encoded_new.type(self.dtype)
            return fea_for_encoded_new, image_features
        if self.ood and self.task_inc and not is_train:
            if len(task_id.unique()) == 1:
                # import pdb; pdb.set_trace()
                cls = self.all_classifer[int(task_id[0])]
                image_features, in_fea = self.image_encoder(image.type(self.dtype))
                fea_for_encoded = image_features.type(torch.FloatTensor).to(self.device)
                fea_for_encoded_new = cls(fea_for_encoded)
                fea_for_encoded_new = fea_for_encoded_new.type(self.dtype)
                res = fea_for_encoded_new.cpu().argmax(dim=1)
                real_res = []
                for va in res:
                    move = task_id[0]*10
                    real_res.append(va + move)
                return real_res, image_features
            else:
                unique_value = task_id.unique()
                out = torch.zeros(len(image))
                for value in unique_value:
                    
                    value = int(value)
                    index = [i for i,val in enumerate(task_id) if val==value]
                    inputs = image[index]
                    cls = self.all_classifer[value]
                    image_features, in_fea = self.image_encoder(inputs.type(self.dtype))
                    fea_for_encoded = image_features.type(torch.FloatTensor).to(self.device)
                    fea_for_encoded_new = cls(fea_for_encoded)
                    fea_for_encoded_new = fea_for_encoded_new.type(self.dtype)
                    res = fea_for_encoded_new.cpu().argmax(dim=1)
                    ...
                    for va in res:
                        move = value*10
                        real_res.append(int(va + move))
                    # import pdb; pdb.set_trace()
                    out[index] = torch.Tensor(real_res)
                # import pdb; pdb.set_trace()
    ...
    def adaptation(self, task_id, increment):
        ...
        if self.linear_probe:
            if self.model_type2 == 'ViT-B/16':
                ...
                self.classifer = ClipMapping(1024, self.cur_cls).to(self.device)
        ...
        #     self.classifer = ClipMapping(512, self.cur_cls).to(self.device)
        elif self.ood and not self.task_inc:
            if self.only_clip:
        ...
        elif self.ood and self.task_inc:
            # self.all_classifer.append(self.classifer)
            self.classifer = ClipMapping(512, increment).to(self.device)
        self.current_class_names = self.classes_names[:self.cur_cls]
    ...
    def get_fea_for_mem(self, loader, task_id):
        ...
        if self.model_type2 == 'ViT-B/16':
            ...
            all_fea = torch.empty((num_of_img, 1024))
        idx = 0
        with torch.no_grad():
    ...
    def load_checkpoint(self):
        clip_m = torch.load('/home/ubuntu/code/clip_based/tiny_task0.pt')
        self.clip_mapping = clip_m.to(self.device)
    ...
    def analyze_mean_cov(self, features, labels):
        ...
        for l in label:
            ...
            # pdb.set_trace()
            # cov = torch.zeros((features.shape[1], features.shape[1]), dtype=features.dtype, device=features.device)
            # for i in range(features.shape[0]):
            #     cov += torch.outer(features[i]-mean, features[i]-mean)
            # cov = cov / features.shape[0]
            ...
            # pdb.set_trace()
            # class_data = class_data[order_index]
            # order_index = order_index[-10:]
            # farest_data = class_data[order_index]
            # farest_dist = torch.cdist(farest_data - mean.unsqueeze(1)).squeeze()
    ...
    def post_train(self, task_id, increment):
        ...
        if self.ood and not self.task_inc:
            ...
            cur_classifer = deepcopy(self.classifer)
            self.all_classifer.append(cur_classifer)
